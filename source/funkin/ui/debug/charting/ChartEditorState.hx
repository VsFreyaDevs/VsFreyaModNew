...
class ChartEditorState extends UIState // UIState derives from MusicBeatState
{
   ...
   */
  public static var STACK_NOTE_THRESHOLD:Int = 10;
  ...
  public function loadPreferences():Void
  {
    ...
    if (previousWorkingFilePaths[0] == null) previousWorkingFilePaths = [null].concat(save.chartEditorPreviousFiles);
    else
  ...
  public function populateOpenRecentMenu():Void
  {
    ...
    for (chartPath in previousWorkingFilePaths)
    {
      ...
      menuItemRecentChart.onClick = (_event) -> {
        ...
        if (result != null)
        {
          if (result.length == 0) this.success('Loaded Chart', 'Loaded chart (${chartPath.toString()})');
          else
  ...
  function buildAdditionalUI():Void
  {
    ...
    txtCopyNotif.setFormat(Paths.font('roboto/robotoBl.ttf'), 24, 0xFF09FF00, CENTER);
    ...
    FlxMouseEvent.add(healthIconBF, (_) -> {
      if (!isCursorOverHaxeUI) this.openCharacterDropdown(CharacterType.BF, true);
    });
    ...
    buttonSelectOpponent.onClick = (_) -> {
      ...
      if (FlxG.keys.pressed.SHIFT) performCommand(new SelectItemsCommand(notesToSelect, []));
      else
    ...
    buttonSelectEvent.onClick = (_) -> {
      if (FlxG.keys.pressed.SHIFT) performCommand(new SelectItemsCommand([], currentSongChartEventData));
      else
  ...
  function handleNoteDisplay():Void
  {
    if (noteDisplayDirty)
    {
      ...
      // Only at Numerator of 1 do we need to extend the view area.
      // who tf makes songs with 1 beat in a measure bruh
      ...
      for (noteData in currentSongChartNoteData)
      {
        ...
        var holdNoteFactory = () -> return new ChartEditorHoldNoteSprite(this); // TODO: Print some kind of warning if `renderedHoldNotes.members` is too high?
