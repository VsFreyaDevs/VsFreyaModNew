...
class ChartEditorNoteDataToolbox extends ChartEditorBaseToolbox
{
  ...
  public override function refresh():Void
  {
    ...
    if (chartEditorState.noteParamsToPlace == null) return;
    ...
    for (pair in chartEditorState.noteParamsToPlace.keyValueIterator())
    {
      ...
      if (field == null) throw 'ChartEditorToolboxHandler.refresh() - Field "${fieldId}" does not exist in the event data form for kind ${previousNoteKind}.';
      else
      {
        switch (field)
        {
  ...
  {
    if (chartEditorState.noteKindToPlace == previousNoteKind) return;
    ...
    for (field in schema)
    {
      ...
      input.onChange = (event:UIEvent) -> {
        ...
        if (value == null) chartEditorState.noteParamsToPlace.remove(event.target.id);
        else
