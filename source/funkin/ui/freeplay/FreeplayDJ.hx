...
class FreeplayDJ extends FlxAtlasSprite
{
  ...
  public function new(x:Float, y:Float, characterId:String)
  {
    ...
    FlxG.console.registerFunction("freeplayCartoon", () -> currentState = Cartoon);
  }
  ...
  public override function update(elapsed:Float):Void
  {
    ...
    {
      if (FlxG.keys.justPressed.LEFT) this.offsetX -= FlxG.keys.pressed.ALT ? 0.1 : (FlxG.keys.pressed.SHIFT ? 10.0 : 1.0);

      if (FlxG.keys.justPressed.RIGHT) this.offsetX += FlxG.keys.pressed.ALT ? 0.1 : (FlxG.keys.pressed.SHIFT ? 10.0 : 1.0);

      if (FlxG.keys.justPressed.UP) this.offsetY -= FlxG.keys.pressed.ALT ? 0.1 : (FlxG.keys.pressed.SHIFT ? 10.0 : 1.0);

      if (FlxG.keys.justPressed.DOWN) this.offsetY += FlxG.keys.pressed.ALT ? 0.1 : (FlxG.keys.pressed.SHIFT ? 10.0 : 1.0);

      if (FlxG.keys.justPressed.C) currentState = (currentState == Idle ? Cartoon : Idle);

      if (FlxG.keys.justPressed.X) currentState = (currentState == Idle ? IdleEasterEgg : Idle);
    }
  }
