...
class CharSelectSubState extends MusicBeatSubState
{
  ...
  override public function create():Void
  {
    ...
    for (id => char in availableChars)
    {
      ...
      if (char == FreeplayState.rememberedCharacterId) moveCursorIndex(id);
    }
  ...
  override public function update(elapsed:Float):Void
  {
    ...
    final moved:Bool = (controls.UI_UP_R || controls.UI_DOWN_R || controls.UI_LEFT_R || controls.UI_RIGHT_R #if mobile || SwipeUtil.swipeUp
      || SwipeUtil.swipeDown || SwipeUtil.swipeLeft || SwipeUtil.swipeRight #end)
      && allowedMovement;
    ...
    if (activeControls)
    {
      if (moved)
      {
        if (validChar) curChar = availableChars.get(getCurrentSelected());
        else
          curChar = "locked";
      }
      ...
      else if (allowInput && controls.BACK)
      {
        cursorConfirmed.visible = false;
