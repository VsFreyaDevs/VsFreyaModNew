...
class ZipUtil
{
  public static var ignoreFilenames:Array<String> = [
    ".git",
    ".gitignore",
    ".github",
    ".vscode",
  ...
  public static function uncompressZip(zip:Reader, destFolder:String, ?prefix:String, ?prog:ZipProgress):ZipProgress
  {
    ...
    try
    {
      if (prefix != null)
      {
        ...
        for (field in f)
          if (field.fileName.startsWith(prefix)) fields.push(field);
      }
      ...
      for (k => field in fields)
      {
        ...
        if (isFolder) FileSystem.createDirectory('${destFolder}/${field.fileName}');
        else
        {
  ...
  public static function writeFolderToZip(zip:ZipWriter, path:String, ?prefix:String, ?prog:ZipProgress, ?whitelist:Array<String>):ZipProgress
  {
    ...
    try
    {
      ...
      (doFolder = () -> {
        ...
        {
          if (ignoreFilenames.contains(e.toLowerCase()) && !whitelist.contains(e.toLowerCase())) continue;
