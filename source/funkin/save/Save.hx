...
class Save
{
  ...
  static function loadFromSlot(slot:Int):Save
  {
    ...
    switch (FlxG.save.status)
    {
      ...
      case BOUND(_, _):
        if (FlxG.save.data != "{ }" && FlxG.save.data != "{}" && FlxG.save.data != null) trace('[SAVE] ' + FlxG.save.data);
        else
...
typedef SaveDataOptions =
{
  ...
  /**
   * If enabled, makes the graphics look sharper, however it may reduce performance.
   * @default `true`
   */
