...
class Bopper extends StageProp implements IPlayStateScriptedClass
{
  ...
  {
    if (_realDanceEvery > 0 && (event.step % (_realDanceEvery * Constants.STEPS_PER_BEAT)) == 0) dance(shouldBop);
  }
  ...
  function update_danceEvery():Void
  {
    ...
    if (FlxMath.getDecimals(danceEvery) == 0) // For the int `danceEvery`.
    {
    ...
    else // For decimal `danceEvery` (x.25, x.50 and x.75)
    {
      // Maybe to rework, for the moment it tries to have the same pattern every measure.
      _realDanceEvery = danceEvery;
