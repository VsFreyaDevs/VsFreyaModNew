...
class PlayState extends MusicBeatSubState
{
  ...
  **/
  ...
  public static var ratingStuff:Array<Dynamic> = [
    ["skill issue", 0.2], // From 0% to 20%
    ["its not overcharted, youre just bad", 0.2], // From 20% to 40%
    ["HA LMAO", 0.4], // From 40% to 60%
  ...
  /**
   * Multiplier for how slow should the camera zoom back.
   * Lerped back to 1.0x every frame.
   */
  ...
  **/
  ...
  public override function create():Void
  {
    ...
    if (Preferences.judgeCounter) add(judgementCounter);
  ...
  public override function update(elapsed:Float):Void
  {
    ...
    {
      cameraBopMultiplier = FlxMath.lerp(1.0, cameraBopMultiplier, 0.95 * cameraZoomingDecay); // Lerp bop multiplier back to 1.0x
  ...
  override function beatHit():Bool
  {
    ...
    if (FlxG.sound.music != null)
    {
      ...
      {
        #if debug trace("NO VOCAL SYNC NEEDED"); #end
        #if debug if (vocals != null) trace(vocals.checkSyncError(correctSync)); #end
        #if debug trace(FlxG.sound.music.time); #end
        #if debug trace(correctSync); #end
      }
  ...
  function initCameras():Void
  {
    ...
    camCutscene = new FunkinCamera('playStateCamHUD');
  ...
  /**
   * Initializes the health bar and info displays on the HUD.
   */
  ...
  function buildDiscordRPCDetails():String
  {
    ...
    {
      if (isChartingMode) return 'Chart Editor [Playtesting]';
      else if (isPracticeMode) return 'Freeplay [Practice]';
      else if (isBotPlayMode) return 'Freeplay [Botplay]';
      else
        return 'Freeplay';
    }
  }
   ...
   */
  public function applyScore(score:Float, daRating:String, healthChange:Float, isComboBreak:Bool, ?latency:Float = 0):Void
  {
    switch (daRating)
    {
      case 'sick':
  ...
  public function resetCamera(?resetZoom:Bool = true, ?cancelTweens:Bool = true, ?snapCamera:Bool = true):Void
  {
    ...
    if (snapCamera) FlxG.camera.focusOn(cameraFollowPoint.getPosition());
  }
