...
class FlxGraphic implements IFlxDestroyable
{
  ...
  public function addFrameCollection(collection:FlxFramesCollection):Void
  {
    if (collection.type != null)
    {
      ...
      if (collections.contains(collection)) FlxG.log.warn('Attempting to add already added collection');
      else
  ...
  function getBitmapFromSystem():BitmapData
  {
    ...
    else if (assetsKey != null) newBitmap = FlxAssets.getBitmapData(assetsKey);
  ...
  {
    if (useCount <= 0 && destroyOnNoUse && !persist) FlxG.bitmap.remove(this);
  }
  ...
  {
    if (imageFrame == null) imageFrame = FlxImageFrame.fromRectangle(this);
